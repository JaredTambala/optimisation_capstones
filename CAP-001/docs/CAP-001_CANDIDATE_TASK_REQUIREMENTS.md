...
- organise machine evidence by per-run core evidence, per-recursive-run
  evidence and comparison-set evidence as defined by the evidence contract;
- make all 25 masters versioned and inspectable, with governed authoring
  mandatory for Incoterms and the decision-material cost, capacity, demand,
  logistics, contract/approval and opening-inventory facts;
- permit creation of a schema-valid Incoterm abstraction with a unique code and
  the same effective-dating, authority and referential controls as other terms;
- treat `STRESS_ONLY` as an optional diagnostic rather than a mandatory product
  behaviour;
- demonstrate a resilience intervention, one within-stage sensitivity and both
  a rejected and an authorised override;
- permit asynchronous and strictly version-pinned, stale-safe stored results,
  while keeping the controlled data journeys live and requiring at least one
  application-launched user-authored solve;
- accept the two-second non-solver interaction and Chromium 1280 x 720
  presentation minimums; and
