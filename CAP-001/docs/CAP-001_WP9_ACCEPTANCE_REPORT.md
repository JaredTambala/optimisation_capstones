...
- Configuration `0.3.6`, data contract `0.3.3` and schema `0.4.0` implement
...
| Repository regression | `pytest -q` | PASS — 99 tests |
...
`evaluation/AI_SUBMISSION_REVIEW_SYSTEM_PROMPT.md` is the
...
## 7. Repository-layout amendment

On 28 August 2026, CN-006 consolidated every CAP-001-specific artefact beneath
the repository's top-level `CAP-001/` project directory and moved the shared
control standard to `standards/`. Configuration `0.3.6` records the new
project-relative control paths. The candidate pack, business data, formulation,
rubric and benchmark controls are unchanged; their manifests and hashes were
regenerated and all WP9 acceptance routes re-run from the new project root.
