...
def test_generated_schema_counts_and_strict_rows() -> None:
    ...
    assert (len(raw_schemas), len(output_schemas), len(fixture_schemas)) == (26, 15, 2)
...
def test_manifests_and_dictionary_expose_controlled_versions() -> None:
    ...
    assert dictionary.count("### `") == 41
    assert "`baseline_standard_costs.csv`" in dictionary
    assert "`recursive_cost_reconciliation.csv`" in dictionary
...
def test_submission_yaml_declares_all_five_commands() -> None:
    ...
    for command in ("setup:", "test:", "solve_baseline:", "solve_default:", "run_app:"):
        assert command in text
    for script in ("setup.sh", "run_tests.sh", "run_baseline.sh", "run_model.sh", "run_app.sh"):
